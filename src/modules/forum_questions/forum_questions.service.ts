...
export class ForumQuestionsService {
  constructor(
    private readonly forumQuestionRepository: ForumQuestionRepository,
  ...
  async postComment(
    id: string,
    { content, userId }: CreateCommentForumQuestionsDto,
  ) {
  ...
  async findMany({ skip, take, search }: FindManySharedDto) {
    ...
    const [total, records] = await Promise.all([
      ...
      (
        await this.forumQuestionRepository.findMany({
          where,
          skip,
          take,
          select,
        ...
        if (question?.question_technologies_tags?.length > 0) {
          //@ts-ignore
          question.question_technologies_tags =
          //@ts-ignore
            question?.question_technologies_tags?.map((obj) => {
